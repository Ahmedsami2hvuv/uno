...
async def turn_timeout_2p(room_id, bot, expected_turn):
    try:
        ...
        msgs = {
            ...
            opp_id: f"⏰ {p_name} ما لعب بالوقت! سحب ورقة والدور رجع الك ✅"
        }
...
async def challenge_timeout_2p(room_id, bot, opp_id, chosen_color, challenge_msg_id):
    try:
        ...
        alerts = {
            players[p_idx]['user_id']: "⏰ الخصم ما رد بالوقت! قبل السحب تلقائياً والدور رجع الك!",
...
async def color_timeout_2p(room_id, bot, player_id):
    try:
        ...
        if penalty > 0:
            ...
            next_turn = p_idx
            alerts[opp_id] = f"⏰ {p_name} ما اختار اللون بالوقت! تم اختيار {chosen_color} تلقائياً وسحبك {penalty} ورقة والدور رجع اله!"
            alerts[player_id] = f"⏰ انتهى الوقت! تم اختيار اللون {chosen_color} تلقائياً."
        else:
            next_turn = (p_idx + 1) % 2
            alerts[opp_id] = f"⏰ {p_name} ما اختار اللون بالوقت! تم اختيار {chosen_color} تلقائياً والدور صار الك!"
            alerts[player_id] = f"⏰ انتهى الوقت! تم اختيار اللون {chosen_color} تلقائياً."
