...
async def confirm_end(callback: types.CallbackQuery):
    ...
    await callback.message.answer("🔔 هل تريد إنهاء الجولة؟", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def show_round_results(callback: types.CallbackQuery, state: FSMContext):
    ...
    for p, pts in d['temp_round'].items():
        res_text += f"👤 {p}: +{pts}\n"
        d['scores'][p] += pts
    res_text += f"\n🏆 الفائز {d['current_winner']} حصل على: +{total_loser_points}"
    game_over = any(s >= d['ceiling'] for s in d['scores'].values())
