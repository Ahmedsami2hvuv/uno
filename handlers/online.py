...
async def send_player_hand(user_id, game_id, old_msg_id=None, extra_text="", is_locked=False):
    ...
    if is_my_turn and not is_locked and "🌈" not in game['top_card']:
        ...
        if not has_move: return await auto_draw(user_id, game_id)
    ...
    turn_marker = "🟢 **دورك الآن (20ث)!**" if is_my_turn else f"⏳ دور: **{opp_info['player_name']}**"
    if is_locked: turn_marker = "⏳ **انتظر رد الخصم...**"
...
async def end_game_logic(winner_id, loser_id, game_id):
    ...
    total_pts = sum(get_card_points(c) for c in loser_hand)
    if total_pts == 0: total_pts = 10
    db_query("UPDATE users SET online_points = online_points + %s WHERE user_id = %s", (total_pts, winner_id), commit=True)
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🎲 جولة جديدة", callback_data="mode_random")],[InlineKeyboardButton(text="🏠 القائمة الرئيسية", callback_data="home")]])
    win_text = (f"🏆 **كفو! فزت بالجولة**\n\n💰 نقاط الجولة: `+{total_pts}`\n🏅 رصيدك الكلي: `{winner_data['online_points']}`")
    lose_text = (f"💀 **خسرت أمام {winner_data['player_name']}**\n\n📉 النقاط المسحوبة: `{total_pts}`\n🏅 رصيدك: `{loser_data['online_points']}`")
    try:
        await bot.send_message(winner_id, win_text, reply_markup=kb)
        await bot.send_message(loser_id, lose_text, reply_markup=kb)
    except: pass

# --- 4. نظام السحب التلقائي (المعدل للسيادة) ---
async def auto_draw(user_id, game_id):
    ...
    new_c = deck.pop(0)
    hand.append(new_c)
    # فحص إذا الورقة المسحوبة ترهم على اللون المختار بالجوكر
...
async def start_turn_timer(game_id, user_id):
    ...
    if int(game['turn']) == int(user_id) and game['status'] == 'playing':
        ...
        await bot.send_message(user_id, "⏰ خلص الوقت (20ث)! انتقل الدور.")
...
async def start_random(callback: types.CallbackQuery):
    ...
    if waiting:
        ...
        while "🌈" in top: deck.append(top); random.shuffle(deck); top = deck.pop()
        ...
        asyncio.create_task(start_turn_timer(g['game_id'], g['p1_id']))
    else:
...
async def process_play(c: types.CallbackQuery):
    ...
    if not is_dominating:
        ...
        if not can_p:
            ...
            await send_player_hand(opp_id, g_id, None, "🔔 الخصم حاول يكسر اللون وتعاقب بورقتين!")
            return await send_player_hand(c.from_user.id, g_id, c.message.message_id, "❌ الورقة ما ترهم على اللون المطلوب!")

    if "🌈" in played_card and "➕" in played_card and not is_dominating and len(my_hand) > 1:
        kb = [[InlineKeyboardButton(text="⚔️ تحدي", callback_data=f"chal_{g_id}_{played_card}"), InlineKeyboardButton(text="✅ لا أتحدى", callback_data=f"nochal_{g_id}_{played_card}")]]
    ...
    if not my_hand:
        if "🌈➕4" in played_card:
            ...
            [opp_h.append(deck.pop(0)) for _ in range(4) if deck]
    ...
    db_query(f"UPDATE active_games SET top_card=%s, {'p1_hand' if is_p1 else 'p2_hand'}=%s, deck=%s, turn=%s, {'p1_uno' if is_p1 else 'p2_uno'}=FALSE WHERE game_id=%s", (played_card, ",".join(my_hand), ",".join(deck), next_turn, g_id), commit=True)
    if played_card == "🌈": await ask_color(c.from_user.id, g_id)
        ...
        asyncio.create_task(start_turn_timer(g_id, next_turn))
...
async def handle_challenge(c: types.CallbackQuery):
    ...
    game = db_query("SELECT * FROM active_games WHERE game_id = %s", (g_id,))[0]
    challenger_id = c.from_user.id; is_p2_chal = (int(challenger_id) == int(game['p2_id']))
    ...
    if is_chal:
        if is_cheat:
            ...
            await send_player_hand(player_id, g_id, None, f"✅ الخصم ظلمك وسحب {f_pen} أوراق! كمل."); await send_player_hand(challenger_id, g_id, c.message.message_id, "❌ الخصم صادق وتعاقبت!")
    else:
        if played_card in p_hand: p_hand.remove(played_card)
        s_val = int(played_card[-1]) if "➕" in played_card else 0
        [o_hand.append(deck.pop(0)) for _ in range(s_val) if deck]
        nt = challenger_id if played_card == "🌈" else player_id
        db_query(f"UPDATE active_games SET {'p1_hand' if is_p1_player else 'p2_hand'}=%s, {'p2_hand' if is_p1_player else 'p1_hand'}=%s, deck=%s, top_card=%s, turn=%s, {'p2_uno' if is_p1_player else 'p1_uno'}=FALSE WHERE game_id=%s", (",".join(p_hand), ",".join(o_hand), ",".join(deck), played_card, nt, g_id), commit=True)
    
    if not p_hand: await end_game_logic(player_id, challenger_id, g_id)
    elif played_card == "🌈": await ask_color(player_id, g_id)
    else: await send_player_hand(player_id, g_id, None); await send_player_hand(challenger_id, g_id, None); asyncio.create_task(start_turn_timer(g_id, player_id))

# --- 8. أونو وصيد وألوان ---
@router.callback_query(F.data.startswith("u_"))
async def process_uno(c: types.CallbackQuery):
    g_id = c.data.split("_")[1]; is_p1 = (int(c.from_user.id) == int(db_query("SELECT p1_id FROM active_games WHERE game_id=%s", (g_id,))[0]['p1_id']))
    db_query(f"UPDATE active_games SET {'p1_uno' if is_p1 else 'p2_uno'}=TRUE WHERE game_id=%s", (g_id,), commit=True)
    await c.answer("📢 أونو!"); await send_player_hand(c.from_user.id, g_id, c.message.message_id, "أمنت نفسك.")
...
async def process_catch(c: types.CallbackQuery):
    ...
    v_hand = [h.strip() for h in (game['p2_hand'] if is_p1 else game['p1_hand']).split(",") if h.strip()]
    if len(v_hand) != 1 or (game['p2_uno'] if is_p1 else game['p1_uno']): return await c.answer("❌ مأمن نفسه!")
    deck = game['deck'].split(","); [v_hand.append(deck.pop(0)) for _ in range(2) if deck]
    db_query(f"UPDATE active_games SET {'p2_hand' if is_p1 else 'p1_hand'}=%s, deck=%s, {'p2_uno' if is_p1 else 'p1_uno'}=FALSE WHERE game_id=%s", (",".join(v_hand), ",".join(deck), g_id), commit=True)
    await c.answer("🎯 صيد ناجح!"); await send_player_hand(c.from_user.id, g_id, c.message.message_id, "صدت الخصم!"); await send_player_hand(victim_id, g_id, None, "انصدت!")
...
async def ask_color(u_id, g_id):
    ...
    await bot.send_message(u_id, "🌈 اختر اللون للسيطرة:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def set_color_logic(c: types.CallbackQuery):
    ...
    await c.message.delete(); await send_player_hand(c.from_user.id, g_id, None, f"اختارينا {col}"); await send_player_hand(opp_id, g_id, None, f"⚠️ اللون صار {col}!"); asyncio.create_task(start_turn_timer(g_id, opp_id))
