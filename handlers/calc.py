...
# --- جلب وحفظ اللاعبين من الداتا بيس ---
def get_saved_calc_players():
    res = db_query("SELECT player_name FROM calc_players")
    return [r['player_name'] for r in res] if res else []

def save_new_calc_player(name):
    db_query("INSERT OR IGNORE INTO calc_players (player_name) VALUES (?)", (name,))

def delete_calc_player(name):
    db_query("DELETE FROM calc_players WHERE player_name = ?", (name,))

# --- البداية ---
@router.callback_query(F.data == "mode_calc")
async def start_calc(callback: types.CallbackQuery, state: FSMContext):
    await state.clear()
    saved_p = get_saved_calc_players()
    data = {
        "all_players": saved_p, 
        "selected": [], 
        "ceiling": 0, 
...
async def render_player_manager(message, state):
    ...
    if len(d.get("selected", [])) >= 2:
        kb.append([InlineKeyboardButton(text="➡️ استمرار لضبط السقف", callback_data="go_ceiling")])
    kb.append([InlineKeyboardButton(text="🏠 القائمة الرئيسية", callback_data="home")])
    
    text = "👥 **إدارة لاعبي الحاسبة**\nالبوت يتذكر هؤلاء اللاعبين دائماً:"
...
async def process_new_name(message: types.Message, state: FSMContext):
    ...
    save_new_calc_player(name)
    state_data = await state.get_data()
    d = state_data['calc_data']
    if name not in d["all_players"]: d["all_players"].append(name)
...
async def remove_p(callback: types.CallbackQuery, state: FSMContext):
    name = callback.data.split("_")[1]
    delete_calc_player(name)
...
async def choose_ceiling(callback: types.CallbackQuery, state: FSMContext):
    kb = [[InlineKeyboardButton(text=str(x), callback_data=f"set_{x}") for x in [100, 150, 200]],
          [InlineKeyboardButton(text=str(x), callback_data=f"set_{x}") for x in [250, 300, 350]],
          [InlineKeyboardButton(text=str(x), callback_data=f"set_{x}") for x in [400, 450, 500]]]
...
async def render_keypad(message, state, target, cur_sum):
    kb = [
        ...
        [InlineKeyboardButton(text="🃏 جوكر 1 (10)", callback_data=f"k_{target}_{cur_sum}_10"), InlineKeyboardButton(text="🃏 جوكر 2 (20)", callback_data=f"k_{target}_{cur_sum}_20")],
        [InlineKeyboardButton(text="🌈 ملون (50)", callback_data=f"k_{target}_{cur_sum}_50"), InlineKeyboardButton(text="➕20", callback_data=f"k_{target}_{cur_sum}_20")],
...
async def finish_round_final(callback: types.CallbackQuery, state: FSMContext):
    ...
    # تحديث النقاط: الخاسرون تزيد نقاطهم، والفائز تزيد نقاطه بمجموعهم فوراً
    ...
    # التحقق من نهاية اللعبة فوراً (حتى لو من أول جولة)
    over = any(s >= d['ceiling'] for s in d['scores'].values())
    
    if over:
        # الفائز هو صاحب أعلى نقاط (بما أن الفائز يجمع النقاط)
...
async def next_rnd(callback: types.CallbackQuery, state: FSMContext):
    await render_main_ui(callback.message, state, "جولة جديدة! ارموا الأوراق.")
