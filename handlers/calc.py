...
def get_player_stats(user_id):
    sql = """SELECT player_name, wins, total_points FROM calc_players WHERE creator_id = %s ORDER BY wins DESC, total_points DESC LIMIT 5"""
...
async def start_calc(callback: types.CallbackQuery, state: FSMContext):
    ...
    data = {"all_players": saved_p, "selected": [], "ceiling": 0, "scores": {}, "direction": "CW", "calculated_losers": [], "temp_round": {}, "current_winner": ""}
...
async def render_player_manager(message, state):
    ...
    for p in d.get("all_players", []): kb_list.append([InlineKeyboardButton(text=("✅ " if p in selected else "▫️ ") + p, callback_data="sel_" + p), InlineKeyboardButton(text="🗑️ مسح", callback_data="delp_" + p)])
...
async def render_loser_list(message, state):
    d = (await state.get_data())['calc_data']
    kb = []
    for p in d['temp_round'].keys(): kb.append([InlineKeyboardButton(text=("✅ " if p in d['calculated_losers'] else "⏳ ") + p + " (" + str(d['temp_round'][p]) + ")", callback_data="calcpts_" + p)])
...
async def save_loser_pts(callback: types.CallbackQuery, state: FSMContext):
    ...
    if t not in d['calculated_losers']:
        d['calculated_losers'].append(t)
    await state.update_data(calc_data=d)
    await render_loser_list(callback.message, state)
...
async def finish_round_final(callback: types.CallbackQuery, state: FSMContext):
    ...
    if any(s >= d['ceiling'] for s in d['scores'].values()):
        ...
        try:
            db_query("UPDATE calc_players SET wins = wins + 1, total_points = total_points + %s WHERE player_name = %s AND creator_id = %s", (total_win_points, fw, callback.from_user.id), commit=True)
            res += f"\n🏁 **انتهت اللعبة!**\nالفائز النهائي: **{fw}** 🏆\n(تم تحديث إحصائياتك بنجاح)"
        except Exception:
            res += f"\n🏁 **انتهت اللعبة!**\nالفائز النهائي: **{fw}** 🏆\n(⚠️ خطأ في حفظ الإحصائيات)"
        kb = [[InlineKeyboardButton(text="📊 إحصائيات لواعبي", callback_data="calc_stats")], [InlineKeyboardButton(text="🏠 الرئيسية", callback_data="home")]]
    else:
